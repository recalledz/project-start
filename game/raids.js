...
import { createHorizontalRaid } from './horizontalRaid.js';
...
export function startRaid(config = buildDefaultConfig()) {
  ...
  raidState.raid = createHorizontalRaid({
    orb: config.orb,
    disciples: config.disciples,
    waves: config.waves,
    container: document.querySelector('.raidCardContainer') || document.body,
  ...
  raidState.raid.start();
  raidState.active = true;
  showRaidAlert('Raiders incoming!');
  document.dispatchEvent(new CustomEvent('raid-start', { detail: {} }));
}
